...
obj/cell_automato
	...
	proc/count_cells()
		var/i = 0
		//world<< "я по координатам [x] [y]"
		for(var/obj/cell_automato/C in range(1,src))
			if(C.cell_is_alive)
				i++
			//	world<< "сосед по координатам [C.x] [C.y]"
		//world<< "соседи = [i]"
		return i
obj/floor/cave
	icon = 'caves.dmi'
...
obj/ladder/proc/gen_caves()
	...
	for(var/i = 0; i<num_of_steps; i++)
		cave_cell_sim_step()
	////SIMULATION STEP
	//последовательно дл€ каждого объекта клеток провер€ем по алгоритму состо€ние и убиваем/оживл€ем/не трогаем этот объект
	//правила проверки:
	//1 - ≈сли у !живой! клетки меньше 2 соседей, она умирает
	//2 - ≈сли у !живой! клетки 2 или 3 соседа, она остаетс€ живой
	//3 - ≈сли у !живой! клетки больше 3 соседей, она умирает
	//4 - ≈сли у !мертвой! клетки ровно 3 соседа, она оживает

	//–исуем пещеры, удал€ем автоматы
...
proc/cave_cell_sim_step()
	...
	/*
	нат€гиваем гриду процессинга на карту, процессим, записыва€ изменени€
	т€€€к, как же нат€нуть массив состо€ний на клеточки
	*/
